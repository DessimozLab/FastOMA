...
def read_infer_xml_rhogs_batch(rhogid_batch_list, inferhog_concurrent_on, batch_folder, big_rest):

    # for now each contain one rhog
    hogs_rhog_xml_batch = []
    for rhogid_num in rhogid_batch_list:
        hogs_rhogs_xml = read_infer_xml_rhog_v2(rhogid_num, inferhog_concurrent_on, batch_folder, big_rest)  # orthoxml_to_newick.py list of hog object
        hogs_rhog_xml_batch.extend(hogs_rhogs_xml)

    return hogs_rhog_xml_batch  # orthoxml_to_newick.py list of hog object


def read_infer_xml_rhog_v2(rhogid_num , inferhog_concurrent_on, folder, big_rest):
