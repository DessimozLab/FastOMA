...
from gethog3.zoo.wrappers import WrapperError
from gethog3.zoo.wrappers.aligners.base_aligner import identify_input
...
def guess_datatype(alignment, from_filename=False):
    ...
    if is_dna(alignment):
        logger.warning("Guessed datatype=DNA. But better explicitly specify the sequence type with option datatype={DNA, PROTEIN}.")
        return DataType.DNA 
    else:
        logger.warning("Guessed datatype=PROTEIN. But better explicitly specify the sequence type with option datatype={DNA, PROTEIN}.")
