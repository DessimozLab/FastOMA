...
from wrappers.seq_utils.utils import iter_seqrecs_from_any
...
def set_default_dna_options(aligner):
    ...
    aligner.options['--nuc'].set_value(True)
...
def set_default_protein_options(aligner):
    ...
    aligner.options = get_default_options()
    aligner.options['--auto'].set_value(True)
    aligner.options['--amino'].set_value(True)
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.input_type == AlignmentInput.OBJECT:  # different operation depending on what it is
            with tempfile.TemporaryDirectory() as tmpdir:
                if self.options['--merge'].active and not os.path.exists(self.options['--merge'].get_value()):
                    self.input, copy_input = itertools.tee(self.input)
                    with open(os.path.join(tmpdir, "submsa_table.txt"), 'wt') as fsub:
                        seq_idx= 1
                        for msa in copy_input:
                            assert isinstance(msa, AlignIO.MultipleSeqAlignment)
                            line = " ".join([str(c + seq_idx) for c in range(len(msa))])
                            fsub.write(line + "\n")
                            seq_idx = seq_idx + len(msa)

                    self.options['--merge'].set_value(fsub.name)
...
def get_default_options():
    return OptionSet([
        ...
        # use unusual characters (e.g., U as selenocysteine in protein sequence; i as inosine in nucleotide sequence), Default=False
        FlagOption('--anysymbol', False, active=False),
