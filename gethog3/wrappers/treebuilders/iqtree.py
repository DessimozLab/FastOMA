...
from wrappers.treebuilders.parsers import IqtreeParser
from wrappers.treebuilders.base_treebuilder import TreeBuilder, AlignmentInput, DataType
...
from wrappers.file_utils import TempFile, TempDir
...
def set_default_dna_options(treebuilder):
    ...
    treebuilder.options['-st'].set_value('DNA')
...
def set_default_protein_options(treebuilder):
    ...
    """
    treebuilder.options = get_default_options()
    treebuilder.options['-st'].set_value('AA')


class Iqtree(TreeBuilder):

    def __init__(self, input_, *args, **kwargs):
        super(Iqtree, self).__init__(alignment=input_, *args, **kwargs)
    ...
    def _read_result(self, tmpd):
        ...
        expected_outfiles = [os.path.join(tmpd, 'tmp_output.iqtree'),
                             os.path.join(tmpd, 'tmp_output.treefile')]
        parser = IqtreeParser()
        try:
