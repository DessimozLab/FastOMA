...
# from wrappers.treebuilders.base_treebuilder.parsers import PhymlParser
...
def set_default_dna_options(treebuilder):
    ...
    treebuilder.options['-d'].set_value('nt')
...
class Phyml(TreeBuilder):
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.input_type == AlignmentInput.OBJECT:  # different operation depending on what it is
            ...
            # some operations done because phyml can not deal with large filenames that are caused due to a large path
            with os.chdir(path):
